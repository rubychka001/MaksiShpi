...
async def show_home(message: Message, session: dict[str, Any], *, edit: bool = False) -> None:
    ...
    markup = kb.welcome_keyboard(bool(session.get("players")))
    if edit:
        await safe_edit(message, texts.WELCOME, markup)
    else:
...
async def show_category(message: Message) -> None:
    await safe_edit(message, texts.CATEGORY_TEXT, kb.category_keyboard())
...
async def command_sticker_id(message: Message) -> None:
    ...
    await message.answer(
        "Отправьте мне нужный стикер следующим сообщением. Я покажу его <code>file_id</code> для файла .env."
    )
...
async def receive_sticker_id(message: Message) -> None:
    ...
    await message.answer(
        "Скопируйте значение ниже в <code>WELCOME_STICKER_ID</code> или <code>RESULT_STICKER_ID</code>:\n\n"
...
async def callback_reset_yes(callback: CallbackQuery) -> None:
    await answer_callback(callback, "Настройки удалены")
    async with CHAT_LOCKS[callback.message.chat.id]:
...
async def callback_default_name(callback: CallbackQuery) -> None:
    ...
    async with CHAT_LOCKS[callback.message.chat.id]:
        ...
        await accept_draft_name(fake_message, f"Игрок {index + 1}", session)
...
async def callback_remove_menu(callback: CallbackQuery) -> None:
    ...
    async with CHAT_LOCKS[callback.message.chat.id]:
        session = await get_session(callback.message.chat.id)
        if len(session["players"]) <= MIN_PLAYERS:
            await answer_callback(callback, f"В игре должно остаться минимум {MIN_PLAYERS} игрока", show_alert=True)
            return
        await safe_edit(
            callback.message,
...
async def callback_deal(callback: CallbackQuery, bot: Bot) -> None:
    ...
    async with CHAT_LOCKS[chat_id]:
        ...
        session["word"] = choose_word(session["category"], session.get("last_word", ""))
...
async def callback_show_role(callback: CallbackQuery) -> None:
    ...
    async with CHAT_LOCKS[chat_id]:
        ...
        if session["state"] != "reveal_hidden" or session.get("role_visible"):
            await answer_callback(callback, "Роль уже была открыта или экран устарел", show_alert=True)
            return
        index = int(session["current_index"])
        if not 0 <= index < len(session["players"]):
            await answer_callback(callback, "Ошибка очереди игроков. Начните раздачу заново.", show_alert=True)
            return
    ...
    async with CHAT_LOCKS[chat_id]:
        ...
        if next_index < len(session["players"]):
            ...
            sent = await bot.send_message(
                chat_id,
                texts.handoff_text(
                    session["players"][next_index], next_index, len(session["players"])
                ),
                reply_markup=kb.reveal_hidden_keyboard(),
            )
            session["active_message_id"] = sent.message_id
...
async def callback_round_start(callback: CallbackQuery) -> None:
    ...
    async with CHAT_LOCKS[callback.message.chat.id]:
        ...
        if session["state"] != "ready_to_start":
            await answer_callback(callback, "Раунд уже запущен или экран устарел", show_alert=True)
            return
